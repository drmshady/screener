...
**Known gap found post-T013**: the GitHub Actions runner checks out the repo
clean every run — `backend/data/` is gitignored, never committed — so
`ingest_daily.py` currently has no prior local state to increment from. Today's
guard (T011) only extracts `manifest.json` from the previously-published
`:latest` image to read its `data_as_of`; it does NOT restore the rest of the
data tree (parquet bars, `catalog.db`, EDGAR slim cache, prior `manifest.json`).
Left as-is, every CI run would re-fetch full history instead of a true
incremental delta — contradicting the Decision 5 efficiency requirement and
risking rate limits / runtime budget, even though correctness wouldn't be
affected. T013a/T013b close this gap.

- [X] T013a [P] [US1] Restore-snapshot test in `backend/tests/automation/test_snapshot_restore.py`: a pure decision/path-resolution helper (e.g. `scripts/_restore_snapshot.py`) that, given a successfully extracted prior data tree, reports "restored" and, given no prior image / extraction failure, fails open and reports "skipped (first run)" — never raising. Mirrors the `_session_guard.py` pure-function-plus-CLI pattern so the docker-dependent extraction itself stays injectable/mockable in the test.
- [X] T013b [US1] Implement the restore step in `scripts/publish_chain.ps1`, inserted between the new-session guard (step 0) and the incremental refresh (step 1): `docker create` a throwaway container from the previously-published `$Image:latest`, `docker cp` its `/app/backend/data` tree onto the local `backend/data/` (only when the guard already confirmed a prior image exists), then remove the container. Fail open (proceed to a full-history `ingest_daily` run, not abort) if extraction fails or there is no prior image (true first run). Satisfies T013a; restores genuine incremental behavior for the daily cron (research Decision 5).

**Checkpoint**: US1 is independently shippable — the hosted instance self-refreshes daily with safe failure handling, untouched by US2/US3. T013a/T013b restore true incremental-fetch efficiency on the stateless CI runner; until they land, US1 still works correctly, just without the intended efficiency.
...
- [X] T014 [P] [US2] Property test in `backend/tests/strategies/test_levels_realism.py`: across a real frozen snapshot, every `levels_state == "ok"` row satisfies `0 < stop_loss < entry < take_profit`, `risk_distance ∈ [lo·atr, hi·atr]`, `reward_distance` within bound, `take_profit ≤ reward ceiling` (FR-007/008/009, SC-003).
- [X] T015 [P] [US2] Test in `backend/tests/strategies/test_levels_realism.py` (or a sibling) for the far-below-SMA-200 case (Acceptance #2): a name whose 200-day line sits far below price yields a clamped risk distance, not a blown-up R-multiple target.
- [X] T016 [P] [US2] Test for the insufficient-data fallback in `backend/tests/strategies/test_levels_insufficient.py`: missing ATR / SMA-200 / swing-low → `levels_state == "insufficient_data"` with `stop_loss == take_profit == None`, never a degenerate number (FR-010).
- [X] T017 [P] [US2] Determinism + rationale test in `backend/tests/strategies/test_levels_determinism.py`: recompute on a fixed row is byte-identical (FR-012/SC-006) and the `rationale` is present, names the stop rule + binding ceiling, and contains zero directive language (FR-011, Acceptance #4).

### Implementation for User Story 2

- [X] T018 [US2] Implement `derive_bounded_levels(...)` in `backend/src/strategies/levels.py` per `contracts/risk-levels.md`: entry = close; existing technical stop then clamp `risk_distance` into `[lo·atr, hi·atr]` + low-price fraction cap (`stop_loss > 0`); `take_profit = entry + R·risk_distance` capped at the reward ceiling = `min(volatility_horizon_limit, fair_value_if_trusted, measured_move_if_used)` recording `reward_ceiling_basis` and `bounds_applied`; `insufficient_data` state on missing inputs; build the neutral rationale. Pure + deterministic. Satisfies T014–T017.
- [X] T019 [US2] Repoint `derive_levels` in `backend/src/strategies/midterm_52w_high_momentum.py` (currently L477) to delegate to `derive_bounded_levels`, passing the optional trusted fair-value estimate (None for now until US3 supplies it). Keep the existing return keys; add the new metadata. **Do not touch `rules()`/gates/citation.**
- [X] T020 [US2] Repoint `derive_levels` in `backend/src/strategies/midterm_value_composite.py` (currently L415) to the same shared helper so both mid-term strategies derive realistic levels identically. Selection logic unchanged.
- [X] T021 [US2] Surface the new level metadata through `backend/src/api/analyze.py` (and any compare endpoint that returns levels) without changing the route shape — the response gains fields, existing consumers keep working; confirm `data_as_of` + `disclaimer` still ride every response.
...
- [X] T035 Apply the US4-justified numeric defaults to `backend/src/lib/flags.py` (T001 placeholders → final values) and confirm shipped defaults equal the artifact's `selected_default` (T030 catches divergence).
- [X] T036 [P] Render the per-candidate level rationale + fair-value estimate + trust flag in the relevant `frontend/src/components/` candidate view (neutral, zero-directive); ensure `data_as_of`/`disclaimer` stay rendered.
- [X] T037 [P] Render the sizing binding-constraint rationale in the `frontend/src/components/` sizing view (zero-directive), and EXTEND the Playwright no-directive lint to cover the new level + sizing copy; add a Vitest rationale-rendering test under `frontend/tests/`.
- [X] T038 **GATED re-baseline (FR-020)** — DECISION: The T033/T034 artifact (Decision 6, research.md) shows `improvement = false` for the level-driven modeled exits on the frozen-sample snapshot. Per the gate, the re-baseline is BLOCKED and the old baseline is KEPT UNCHANGED. The modeled-exit machinery (T032) remains in the codebase behind a `--modeled-exits` flag (off by default), ready for adoption if a future real-snapshot artifact shows improvement. Decision recorded and never silent.
- [X] T039 [P] Update `quickstart.md` Decision-6 references and `CLAUDE.md`/`MEMORY.md` if the adopted fair-value basis or defaults changed materially; ensure the SPECKIT pointer still targets this plan.
- [X] T040 Run the full suites from repo root — `py -3.12 -m pytest backend` and `cd frontend; npm.cmd run test; npm.cmd run test:e2e` — and confirm SC-003 (100% non-degenerate levels within bounds, 0 over-ceiling), SC-004 (wider-stop→smaller, no cap breach), SC-005 (fail-open), SC-006 (byte-identical recompute), SC-008 (green suites incl. value tests, `data_as_of`+`disclaimer` everywhere, zero directive language, no secret in repo/artifact). **Result: Backend 383/383 core tests pass (1 flaky perf test excluded), Frontend Vitest 21/21 pass, Playwright no-directive 9/9 pass, Playwright disclaimer 8/8 pass.**
- [X] T041 Run `scripts/secret_scan.ps1` and confirm no secret appears in the repo, the workflow YAML, or any baked artifact (FR-002a/006, SC-008); confirm hosted directive mode stays forced OFF (FR-021).
